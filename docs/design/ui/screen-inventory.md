...
| P0 | `04b Settings / Appearance` | 切换主题与语言 | 跟随系统 / 浅色 / 深色；中文 / 英文；**即时生效，与需重启的配置项行为不同** | CG-11（存 localStorage）、CG-12；纯前端，无 API |
...
- 设置页存在两类行为不同的设置：外观即时生效，其余需重启。界面必须让这个差别可见，不得画成同样的行配同样的保存按钮。
